...
if [ ! -f "$WORDPRESS_HOME/wp-config.php" ]; then
	...
	set_var_if_null "WORDPRESS_DB_USER" "wordpress"
	...
	if [ $WORDPRESS_DB_HOST == "127.0.0.1" ]; then
		...
		set_var_if_null 'PHPMYADMIN_USER' 'phpmyadmin'
		...
		mysql -u root -e "create database $WORDPRESS_DB_NAME; grant all on $WORDPRESS_DB_NAME.* to '$WORDPRESS_DB_USER'@'127.0.0.1' identified by '$WORDPRESS_DB_PASSWORD'; flush privileges;"

		# create database user for phpMyAdmin
		mysql -u root -e "create user '$PHPMYADMIN_USER'@'127.0.0.1' identified by '$PHPMYADMIN_PASSWORD'; grant all on *.* to '$PHPMYADMIN_USER'@'127.0.0.1' with grant option; flush privileges;"	
	...
	sed -i "s/connectstr_dbusername = '';/connectstr_dbusername = '$WORDPRESS_DB_USER';/" "$WORDPRESS_HOME/wp-config.php"
